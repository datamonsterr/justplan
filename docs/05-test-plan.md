...
describe('scheduleTasksGreedy', () => {
  ...
  it('should add buffer time between tasks', () => {
    ...
    const schedule = scheduleTasksGreedy(tasks, availability, { bufferMinutes })
...
describe('calculateAvailability', () => {
  ...
  it('should exclude existing events from availability', () => {
    ...
    const date = nextMonday(new Date())
    const busyTimes = [
      { start: setHours(date, 10), end: setHours(date, 11) }
    ]
  ...
  it('should respect timezone', () => {
    ...
    const slots = calculateAvailability(date, workingHours, [], date, 'America/New_York')
...
describe('useTasks', () => {
  ...
  it('should filter tasks by state', async () => {
    ...
    await waitFor(() => {
      expect(result.current.tasks.every(t => t.state === 'in-progress')).toBe(true)
    })
  })
  ...
  it('should handle optimistic updates', async () => {
    ...
    expect(result.current.tasks.find(t => t.id === 'task-id').priority).toBe('high')
  })
})
```
...
describe('GoogleCalendarClient', () => {
  ...
  describe('listEvents', () => {
    ...
    it('should handle authentication errors', async () => {
      ...
      await expect(client.listEvents('primary', {}))
  ...
  describe('createEvent', () => {
    ...
    it('should batch multiple event creations', async () => {
      const events = [
        { summary: 'Task 1', start: '2026-02-17T10:00:00Z', end: '2026-02-17T11:00:00Z' },
        { summary: 'Task 2', start: '2026-02-17T14:00:00Z', end: '2026-02-17T15:00:00Z' }
      ]
...
test.describe('Task Management', () => {
  test('complete task lifecycle', async ({ authenticatedPage: page }) => {
    ...
    // Edit task
    await page.click('[data-testid="task-item"]:has-text("Complete project documentation")')
    ...
    await expect(page.locator('[data-testid="task-state"]')).toHaveValue('In Progress')
    
    // Complete task
    await page.click('button:has-text("Mark Complete")')
    await expect(page.locator('[data-testid="task-state"]')).toHaveValue('Done')
    ...
    await expect(page.locator('text=Complete project documentation')).not.toBeVisible()
  })
})
```
...
test.describe('Auto-Scheduling', () => {
  test('schedule tasks automatically', async ({ authenticatedPage: page }) => {
    ...
    await expect(page.locator('text=Scheduling complete')).toBeVisible({ timeout: 10000 })
    
    // View calendar
    await page.click('a:has-text("Calendar")')
    
    // Verify tasks appear on calendar
    await expect(page.locator('[data-testid="calendar-event"]:has-text("Task 1")')).toBeVisible()
    await expect(page.locator('[data-testid="calendar-event"]:has-text("Task 2")')).toBeVisible()
    await expect(page.locator('[data-testid="calendar-event"]:has-text("Task 3")')).toBeVisible()
    
    // Verify scheduled within working hours
    const event1 = await page.locator('[data-testid="calendar-event"]:has-text("Task 1")').getAttribute('data-start-time')
    expect(parseInt(event1)).toBeGreaterThanOrEqual(9) // After 9 AM
    expect(parseInt(event1)).toBeLessThanOrEqual(17) // Before 5 PM
  })

  test('manual reschedule via drag and drop', async ({ authenticatedPage: page }) => {
    await page.goto('/calendar')
    
    const task = page.locator('[data-testid="calendar-event"]:has-text("Task 1")')
    const targetSlot = page.locator('[data-testid="calendar-slot"][data-time="14:00"]')
...
test.describe('Workflow Transitions', () => {
  test('automatic state transition on deadline', async ({ authenticatedPage: page }) => {
    ...
    await page.fill('[name="deadline"]', nearDeadline.toISOString().slice(0, 16))
    await page.click('button:has-text("Create")')
    
    // Task should be in "Ready" state initially
    await expect(page.locator('[data-testid="task-state"]')).toHaveText('Ready')
    ...
    await expect(page.locator('[data-testid="task-state"]')).toHaveText('Urgent')
  })

  test('configure custom workflow transition', async ({ authenticatedPage: page }) => {
  ...
  test('sync Google Tasks to app', async ({ authenticatedPage: page }) => {
    ...
    await page.route('**/tasks/v1/lists/*/tasks', route => {
      route.fulfill({
        ...
        body: JSON.stringify({
          items: [
            { id: 'gtask-1', title: 'Imported task from Google' }
          ]
        })
      })
    })
