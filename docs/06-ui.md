...
5. **Flexible Customization**: Resizable panels, custom workflows, theme options
...
- **Panel Resize**: 60fps (16ms frame time)
...
### IDE-Inspired Four-Panel Layout

```mermaid
graph TB
    subgraph "JustPlan Layout"
        Navbar[Top Navbar - 40px height]
        LeftSidebar[Left Sidebar<br/>AI Copilot<br/>250-500px]
        Center[Center Area<br/>Calendar View<br/>Flexible width]
        RightSidebar[Right Sidebar<br/>Task List<br/>300-600px]
        BottomPanel[Bottom Panel - Auto-hide<br/>Time Block Details<br/>200px-60vh]
        
        Navbar --> LeftSidebar
        Navbar --> Center
        Navbar --> RightSidebar
        
        LeftSidebar -.Resize Handle.- Center
        Center -.Resize Handle.- RightSidebar
        Center --> BottomPanel
    end
...
| Panel | Default Size | Min Size | Max Size | Resizable | Collapsible |
|-------|--------------|----------|----------|-----------|-------------|
| Navbar | 40px height | Fixed | Fixed | No | No |
| Left AI Sidebar | 320px | 250px | 500px | Yes | Yes (icon-only) |
| Center Calendar | Flexible | 600px | 100% | No | No |
| Right Task Sidebar | 400px | 300px | 600px | Yes | Yes (icon-only) |
| Bottom Panel | 300px | 200px | 60vh | Yes | Yes (slide down) |
...
@media (min-width: 1024px) {
  /* Standard four-panel layout */
}

/* Large Desktop: Wider default panel sizes */
@media (min-width: 1440px) {
  /* Increase default widths for comfort */
}
```
