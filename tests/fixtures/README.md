...
export const taskTemplates = {
  ...
  withDeadline: (daysFromNow: number) =>
    createMockTask({
...
export const generateTimeSlots = (
  ...
  for (let i = 0; i < slotCount; i++) {
    ...
    slots.push({
      start: setMinutes(
        setHours(new Date(), Math.floor(startMinutes / 60)),
        startMinutes % 60
      ),
      end: setMinutes(
        setHours(new Date(), Math.floor(endMinutes / 60)),
        endMinutes % 60
      ),
    });
  }
...
export async function seedTestDatabase(supabase: SupabaseClient) {
  ...
  await supabase
    .from("users")
    .insert([testUsers.powerUser, testUsers.basicUser, testUsers.teamLead]);
